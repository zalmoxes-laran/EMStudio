...
const EN: Dict = {
  ...
  "members.joinNoHub": "This invitation needs an em-server address — set one in Settings ▸ Live sync.",
  ...
  "assets.residentNote": "The bytes go into the room's object store: em-server is in their path, so the embargo gate and the licence header apply to them.",
  "assets.referenceNote": "HONEST NOTE: bytes by reference stay outside em-server, so NO embargo can be applied to them and the licence can only travel, not be enforced. Never use it for sensitive or embargoed material. It needs a path — a file dropped from the desktop has no location the graph could point at.",
...
const IT: Dict = {
  ...
  "members.joinNoHub": "Questo invito ha bisogno dell'indirizzo di un em-server — impostalo in Impostazioni ▸ Live sync.",
  ...
  "assets.residentNote": "I byte entrano nell'object store della stanza: em-server è sul loro percorso, quindi il cancello dell'embargo e l'header della licenza valgono per loro.",
  "assets.referenceNote": "NOTA ONESTA: i byte by reference stanno FUORI da em-server, quindi NESSUN embargo può essere applicato e la licenza può solo viaggiare, non essere imposta. Mai per materiale sensibile o embargato. Serve un percorso: un file trascinato dal desktop non ha una posizione che il grafo possa indicare.",
